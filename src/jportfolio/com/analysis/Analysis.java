...
public class Analysis {
	...
	// assets' returns
	// This method calculated different weighted combination for each portfolio to
	...
	public static double expectedReturnPortfolio(List<Asset> averageReturnList, Double[] weight) {
		...
		for (int i = 0; i < averageReturnList.size(); i++) {
			maxReturn += averageReturnList.get(i).getExpectedReturn() * weight[i]; // double maxReturn = m1*w1 + m2*w2 +
																					// m3*w3 + m4*w4 + ...;
		}
		return maxReturn;
	}
